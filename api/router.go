...
func routeInit(dao *dao.Dao) *gin.Engine {
	...
	// 请求宣传案例
	router.GET("/api/publicityCase/get", controllers.RequestPublicityCase)
	// 确认宣传案例
	router.GET("/api/publicityCase/confirm", controllers.ConfirmPublicityCase)

	// 针对企业微信后台开放接口
	router.POST("/api/wecom/alarm", WeComVictimAlarm)
