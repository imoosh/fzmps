...
type PublicityCaseContentResp struct {
	Content string `json:"content"`
}
...
func RequestPublicityCaseContent(c *gin.Context) {
	id := c.Query("id")
	if len(id) == 0 {
		c.JSON(http.StatusOK, utils.ReturnHTTPSuccess(nil))
		return
	...
	var resp *PublicityCaseContentResp
	pc := DB(c).QueryPublicityCaseContent(caseId)
	resp = &PublicityCaseContentResp{Content: pc.Content}
